...
*********************************************************************************
* $Id: FXPrintDialog.cpp,v 1.31 2002/01/18 22:43:01 jeroen Exp $                *
********************************************************************************/
#include "xincs.h"
#include "fxver.h"
#include "fxdefs.h"
...
/* Generated by reswrap from file landscape.gif */
...
/* Generated by reswrap from file portrait.gif */
...
// Separator item
FXPrintDialog::FXPrintDialog(FXWindow* owner,const FXString& name,FXuint opts,FXint x,FXint y,FXint w,FXint h):
  FXDialogBox(owner,name,opts|DECOR_TITLE|DECOR_BORDER|DECOR_RESIZE,x,y,w,h,0,0,0,0,4,4){
  FXchar key[20],print[100]; const FXchar *paper;
  ...
  printername=new FXComboBox(printdest,25,4,this,ID_PRINTER_NAME,COMBOBOX_NORMAL|FRAME_SUNKEN|FRAME_THICK|LAYOUT_CENTER_Y|LAYOUT_FILL_X);
  ...
  media=new FXListBox(paperdims,6,this,ID_MEDIA,FRAME_SUNKEN|FRAME_THICK|LISTBOX_NORMAL|LAYOUT_FILL_X|LAYOUT_SIDE_TOP);
  ...
  for(int i=0; ; i++){
    sprintf(key,"%d",i);
    paper=getApp()->reg().readStringEntry("PAPER",key,NULL);
    if(!paper) break;
    FXTRACE((100,"Paper %d=\"%s\"\n",i,paper));
    sscanf(paper,"[%[^]]] %*f %*f %*f %*f %*f %*f",print);
    media->appendItem(print);
    }
  ...
  paper=getApp()->reg().readStringEntry("PAPER",key,"[US Letter] 612 792 72 72 72 72");

  // Parse size
  sscanf(paper,"[%[^]]] %lf %lf %lf %lf %lf %lf",print,&printer.mediawidth,&printer.mediaheight,&printer.leftmargin,&printer.rightmargin,&printer.topmargin,&printer.bottommargin);
...
long FXPrintDialog::onUpdPortrait(FXObject* sender,FXSelector,void*){
  if(printer.flags&PRINT_LANDSCAPE)
    sender->handle(this,MKUINT(ID_UNCHECK,SEL_COMMAND),NULL);
  else
    sender->handle(this,MKUINT(ID_CHECK,SEL_COMMAND),NULL);
  return 1;
...
long FXPrintDialog::onUpdPages(FXObject* sender,FXSelector sel,void*){
  ...
  sender->handle(this,MKUINT(ch,SEL_COMMAND),NULL);
  return 1;
  }
...
long FXPrintDialog::onCmdNumCopies(FXObject* sender,FXSelector,void*){
  sender->handle(this,MKUINT(ID_GETINTVALUE,SEL_COMMAND),(void*)&printer.numcopies);
  return 1;
  }
...
long FXPrintDialog::onUpdNumCopies(FXObject* sender,FXSelector,void*){
  sender->handle(this,MKUINT(ID_SETINTVALUE,SEL_COMMAND),(void*)&printer.numcopies);
  return 1;
  }
...
long FXPrintDialog::onCmdFirstPage(FXObject* sender,FXSelector,void*){
  FXint pg;
  sender->handle(this,MKUINT(ID_GETINTVALUE,SEL_COMMAND),(void*)&pg);
  FXASSERT(pg>0);
...
long FXPrintDialog::onUpdFirstPage(FXObject* sender,FXSelector,void*){
  sender->handle(this,MKUINT(ID_SETINTVALUE,SEL_COMMAND),(void*)&printer.frompage);
...
long FXPrintDialog::onUpdLastPage(FXObject* sender,FXSelector,void*){
  sender->handle(this,MKUINT(ID_SETINTVALUE,SEL_COMMAND),(void*)&printer.topage);
  if(printer.flags&PRINT_PAGES_RANGE)
    sender->handle(this,MKUINT(ID_ENABLE,SEL_COMMAND),NULL);
  else
    sender->handle(this,MKUINT(ID_DISABLE,SEL_COMMAND),NULL);
...
long FXPrintDialog::onCmdMedia(FXObject*,FXSelector,void*){
  ...
  sprintf(key,"%d",printer.mediasize);
  paper=getApp()->reg().readStringEntry("PAPER",key,"[US Letter] 612 792 72 72 72 72");
  sscanf(paper,"[%[^]]] %lf %lf %lf %lf %lf %lf",name,&printer.mediawidth,&printer.mediaheight,&printer.leftmargin,&printer.rightmargin,&printer.topmargin,&printer.bottommargin);
...
long FXPrintDialog::onUpdMedia(FXObject*,FXSelector,void*){
  if(printer.mediasize<=media->getNumItems()) media->setCurrentItem(printer.mediasize);
  return 1;
  }
...
FXPrintDialog::~FXPrintDialog(){
  ...
  delete portraitIcon;
  landscapeIcon=(FXIcon*)-1;
  portraitIcon=(FXIcon*)-1;
  }
