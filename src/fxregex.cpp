...
*********************************************************************************
* $Id: fxregex.cpp,v 1.5 1998/10/02 22:57:46 jvz Exp $                        *
********************************************************************************/
#include "xincs.h"
#include "fxdefs.h"
#include "fxregex.h"
...
/*
 * Global work variables for fxregcomp().
 */
...
/*
 * Global work variables for fxregexec().
 */
...
static int  regtry(fxregexp *prog,const char *string);
...
void regerror(const char *s){ fprintf(stderr, "regexp(3): %s", s); }
 ...
 */
fxregexp *fxregcomp(const char* exp){
  register fxregexp *r;
  register char *scan;
  ...
  if(regsize>=32767L) return NULL;

  /* Allocate space. */
  r=(fxregexp*)malloc(sizeof(fxregexp)+(unsigned)regsize);
...
/*
 - fxregfree - frees the program
 */
void fxregfree(fxregexp* prog){
  if(prog) free(prog);
  }
...
static char *reg(int paren,int *flagp){
  ...
  *flagp=HASWIDTH;                        /* Tentatively. */
  ...
  if(paren){
    if(regnpar>=NSUBEXP) return NULL;     /* too many () */
    parno = regnpar;
    regnpar++;
...
static char *regpiece(int *flagp){
  ...
  if(!(flags&HASWIDTH) && op!='?') return NULL;   /* *+ operand could be empty */
  ...
  *flagp = (op!='+') ? (WORST|SPSTART) : (WORST|HASWIDTH);
  ...
  if(ISMULT(*regparse)) return NULL;        /* nested *?+ */
  ...
  *flagp=WORST;         /* Tentatively. */
  ...
  switch(*regparse++){
    ...
    case '[': {
      ...
      while(*regparse!='\0' && *regparse!=']'){
        if(*regparse == '-'){
          ...
          if(*regparse==']' || *regparse=='\0'){
            ...
            if(cclass>classend+1) return NULL;  // invalid [] range
            for(; cclass<=classend; cclass++) regc(cclass);
            regparse++;
            }
          }
        else{
      ...
      regc('\0');
      if(*regparse!=']') return NULL;           // unmatched []
      regparse++;
      *flagp|=HASWIDTH|SIMPLE;
      }
      break;
    ...
    case '|':
    case ')':
      return NULL;                              // internal urp
    case '?':
    case '+':
    case '*':
      return NULL;                              // ?+* follows nothing
    case '\\':
      if(*regparse == '\0') return NULL;        // trailing '\\'
      ret = regnode(EXACTLY);
      regc(*regparse++);
      regc('\0');
      *flagp|=HASWIDTH|SIMPLE;
    ...
    default: {
      ...
      if(len<=0) return NULL;                   // internal disaster 
      ender = *(regparse+len);
 ...
 */
int fxregexec(fxregexp *prog,const char *string){
  ...
  if(UCHARAT(prog->program)!=MAGIC){
    fxerror("fxregexec: corrupted regular expression.");
    return 0;
    }
...
static int regmatch(char *prog){
  ...
  while(scan != NULL){
	...
	break;
      ...
      case END:
	      ...
	      break;
      default:
	      fxerror("fxregexp: memory corruption.\n");
	      return(0);
	      break;
      }
   ...
   */
  fxerror("fxregexec: corrupted pointers.\n");
  return 0;
  }
...
static int regrepeat(char *p){
  ...
  switch(OP(p)){
    ...
    default:                /* Oh dear.  Called inappropriately. */
      fxerror("fxregexec: internal foulup.\n");
...
/*
 - fxregdump - dump a regexp onto stdout in vaguely comprehensible form
 */
void fxregdump(fxregexp *r){
  register char *s;
...
static char *regprop(char *op){
  ...
  switch (OP(op)){
    ...
    case PLUS:
      ...
      break;
    default:
      fxerror("fxregexp: corrupted opcode.\n");
      break;
      }
 ...
 */
void fxregsub(fxregexp* prog,const char* source,char *dest){
  register const char *src;
  register char *dst;
  register char c;
  register int no;
  register int len;
  if(prog==NULL || source==NULL || dest==NULL) return;
  if(UCHARAT(prog->program)!=MAGIC){
    fxerror("fxregsub: damaged regexp fed to regsub.\n");
    return;
    }
  src=source;
  dst=dest;
    ...
    if(no<0)     /* Ordinary character. */
      *dst++=c;
      ...
      if(len!=0 && *(dst-1)=='\0'){             /* strncpy hit NUL. */
        fxerror("fxregsub: damaged match string.\n");
        return;
        }
      }
    }
